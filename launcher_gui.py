...
class Launcher:
    ...
    def start(self):
        self.processes.append(
            subprocess.Popen(f'gnome-terminal -- python3 server.py -p {self.port.get()} -a {self.host.get()}',
                             shell=True))
        time.sleep(0.5)
        for i in range(int(self.clients.get())):
            self.processes.append(subprocess.Popen(
                f'gnome-terminal -- python3 client.py {self.host.get()} {self.port.get()} -n test{i + 1}', shell=True))
